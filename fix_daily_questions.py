...
def fix_daily_questions():
    ...
    try:
        ...
        with engine.connect() as conn:
            result = conn.execute(text("SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='daily_questions' AND column_name='user_id')"))
            user_id_exists = result.scalar()
            
            result = conn.execute(text("SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='daily_questions' AND column_name='account_id')"))
            account_id_exists = result.scalar()
            
            if user_id_exists and not account_id_exists:
                print("Renaming user_id column to account_id in daily_questions table...")
                conn.execute(text("ALTER TABLE daily_questions RENAME COLUMN user_id TO account_id"))
