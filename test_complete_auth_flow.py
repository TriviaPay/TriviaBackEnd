...
def test_complete_auth_flow():
    ...
    try:
        ...
        print(f"   Refresh with expired token: {response.status_code} - {response.text[:100]}")
    ...
    try:
        response = requests.post(f"{base_url}/auth/refresh-new-session", headers=expired_headers)
    ...
    try:
        ...
        print(f"   Bind password with valid token: {response.status_code} - {response.text[:100]}")
        if response.status_code == 200:
    ...
    try:
        ...
        print(f"   Bind password with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code == 401:
    ...
    try:
        ...
        print(f"   Test descope auth with valid token: {response.status_code} - {response.text[:100]}")
        if response.status_code == 200:
    ...
    try:
        response = requests.get(f"{base_url}/test-descope-auth", headers=expired_headers)
        print(f"   Test descope auth with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code == 401:
    ...
    print("In a real environment, you would use actual Descope tokens from the frontend.")
