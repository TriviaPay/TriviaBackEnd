...
class TestListWithdrawals:
    ...
    async def test_list_withdrawals_default(
        self, mock_admin_user, mock_withdrawal_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_withdrawals_non_admin(self):
...
class TestApproveWithdrawal:
    ...
    async def test_approve_withdrawal_success(
        ...
        with patch(
            "app.routers.payments.admin_withdrawals.create_payout",
            return_value={
            ...
            "app.routers.payments.admin_withdrawals.adjust_wallet_balance",
            return_value=10000,
        ):
            async with AsyncClient(app=app, base_url="http://test") as ac:
    ...
    @pytest.mark.asyncio
    async def test_approve_withdrawal_not_found(self, mock_admin_user):
    ...
    async def test_approve_withdrawal_payout_failure(
        ...
        with patch(
            "app.routers.payments.admin_withdrawals.create_payout",
            side_effect=PayoutFailed("Insufficient funds in Stripe account"),
        ):
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.post(
                    "/api/v1/admin/withdrawals/1/approve",
...
class TestRejectWithdrawal:
    ...
    @pytest.mark.asyncio
    async def test_reject_withdrawal_not_found(self, mock_admin_user):
