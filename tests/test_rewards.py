...
def test_get_draw_config():
    ...
    if response.status_code == 200:
        ...
        print(f"  Draw time: {result['draw_time_hour']}:{result['draw_time_minute']} {result['draw_timezone']}")
    else:
        print(f"Failed to get draw configuration: {response.status_code} - {response.text}")
...
def test_update_draw_config():
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to update draw configuration: {response.status_code} - {response.text}")
    ...
    if response.status_code == 200:
        ...
        if result["winners"]:
            ...
            for winner in result["winners"]:
                print(f"  Position {winner['position']}: {winner['username']} - ${winner['prize_amount']}")
    else:
...
def test_get_daily_winners():
    ...
    response = requests.get(
        f"{BASE_URL}/winners/daily-winners",
        headers=get_headers()
    )
    ...
    if response.status_code == 200:
        ...
        for winner in winners:
            print(f"  {winner['username']} - Position: {winner['position']}, Amount: ${winner['amount_won']}, All-time: ${winner['total_amount_won']}")
    else:
...
def test_get_weekly_winners():
    ...
    response = requests.get(
        f"{BASE_URL}/winners/weekly-winners",
        headers=get_headers()
    )
...
def test_get_all_time_winners():
    ...
    response = requests.get(
        f"{BASE_URL}/winners/all-time-winners",
        headers=get_headers()
    )
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to get all-time winners: {response.status_code} - {response.text}")
