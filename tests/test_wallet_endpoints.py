...
class TestWalletBalance:
    ...
    @pytest.mark.asyncio
    async def test_get_wallet_balance_success(self, mock_get_current_user):
        """Test successful wallet balance retrieval"""
        from app.db import get_async_db
        from app.dependencies import get_current_user
        from app.routers.payments.wallet import get_wallet_balance
        ...
        with patch(
            "app.routers.payments.wallet.get_wallet_balance", return_value=100000
        ):
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(
                    "/api/v1/wallet/me", headers={"Authorization": "Bearer test_token"}
                )
    ...
    @pytest.mark.asyncio
    @patch("app.routers.payments.wallet.get_current_user")
    @patch("app.routers.payments.wallet.get_async_db")
    @patch("app.routers.payments.wallet.get_wallet_balance")
    async def test_get_wallet_balance_with_transactions(
        self, mock_get_balance, mock_db, mock_get_user, mock_get_current_user
    ):
...
class TestWithdrawal:
    ...
    @patch("app.routers.payments.wallet.get_current_user")
    @patch("app.routers.payments.wallet.get_async_db")
    @patch("app.routers.payments.wallet.adjust_wallet_balance")
    @patch("app.routers.payments.wallet.get_wallet_balance")
    @patch("app.routers.payments.wallet.calculate_withdrawal_fee")
    @patch("app.routers.payments.wallet.get_daily_instant_withdrawal_count")
    @patch("app.routers.payments.wallet.create_payout")
    async def test_instant_withdrawal_success(
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            response = await ac.post(
                ...
                json={"amount_minor": 10000, "type": "instant"},  # $100.00
                headers={"Authorization": "Bearer test_token"},
            )
    ...
    async def test_withdrawal_no_stripe_account(
        self, mock_get_balance, mock_db, mock_get_user, mock_get_current_user_no_stripe
    ):
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            response = await ac.post(
                "/api/v1/wallet/withdraw",
                json={"amount_minor": 10000, "type": "instant"},
                headers={"Authorization": "Bearer test_token"},
            )
    ...
    async def test_instant_withdrawal_payout_failure_refund(
        ...
        mock_create_payout.side_effect = PayoutFailed(
    ...
    async def test_standard_withdrawal_creates_pending(
        ...
        async with AsyncClient(app=app, base_url="http://test") as ac:
            response = await ac.post(
                "/api/v1/wallet/withdraw",
                json={"amount_minor": 10000, "type": "standard"},
                headers={"Authorization": "Bearer test_token"},
            )
