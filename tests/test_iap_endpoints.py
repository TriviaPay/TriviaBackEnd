...
from app.services.iap_service import verify_apple_receipt, verify_google_purchase, get_product_credit_amount
...
MOCK_APPLE_RECEIPT = base64.b64encode(json.dumps({
...
class TestAppleIAPVerification:
    ...
    async def test_apple_verify_success(
        ...
        with patch('app.routers.iap.verify_apple_receipt', return_value={
            'verified': True,
            'transaction_id': APPLE_TEST_TRANSACTION_ID,
            'product_id': TEST_PRODUCT_GEMS,
        ...
        patch('app.routers.iap.get_product_credit_amount', return_value=49900), \
        patch('app.routers.iap.adjust_wallet_balance', return_value=49900):
    ...
    @pytest.mark.asyncio
    async def test_apple_verify_failed_verification(
        self,
        mock_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_apple_verify_product_not_found(
        self,
        mock_user
    ):
        ...
        patch('app.routers.iap.get_product_credit_amount', return_value=None):
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.post(
                    "/api/v1/iap/apple/verify",
                    json={
    ...
    @pytest.mark.asyncio
    async def test_apple_verify_idempotency(
        self,
        mock_user
    ):
...
class TestGoogleIAPVerification:
    ...
    async def test_google_verify_success(
        ...
        with patch('app.routers.iap.verify_google_purchase', return_value={
            'verified': True,
    ...
    @pytest.mark.asyncio
    async def test_google_verify_failed_verification(
        self,
        mock_user
    ):
