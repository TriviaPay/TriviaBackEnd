...
def test_auth_endpoints():
    ...
    # Test with no token
    try:
        response = requests.post(f"{base_url}/bind-password?password=Fortrivia@1&username=Trivia")
        print(f"   No token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for missing token")
    except Exception as e:
    ...
    try:
        ...
        response = requests.post(f"{base_url}/profile/update", json=profile_data, headers=headers)
        print(f"   Profile update with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        response = requests.post(f"{base_url}/profile/check-username", json=username_data, headers=headers)
        print(f"   Username check with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        print(f"   Admin endpoint with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        print(f"   Wallet balance with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        print(f"   Trivia questions with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        print(f"   Store items with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        print(f"   Stripe payment methods with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        print(f"   Stripe create customer with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        print(f"   Refresh endpoint with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
    ...
    try:
        ...
        print(f"   Refresh new session with expired token: {response.status_code} - {response.text[:100]}")
        if response.status_code != 401:
            print("   ❌ ERROR: Should return 401 for expired token")
    except Exception as e:
        print(f"   ❌ ERROR: Exception with expired token: {e}")
