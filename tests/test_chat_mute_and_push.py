...
def auth_headers_user1(test_user1):
    ...
    yield {"Authorization": "Bearer test_token_user1"}
...
def auth_headers_user2(test_user2):
    ...
    app.dependency_overrides[get_current_user] = _get_user
    yield {"Authorization": "Bearer test_token_user2"}
    app.dependency_overrides.clear()
...
def test_get_mute_preferences_empty(auth_headers_user1, test_user1, test_db):
    ...
    prefs = (
        test_db.query(ChatMutePreferences)
        .filter(ChatMutePreferences.user_id == test_user1.account_id)
        .first()
    )
...
def test_mute_global_chat(auth_headers_user1, test_user1, test_db):
    ...
    response = client.post(
        "/chat-mute/global", headers=auth_headers_user1, json={"muted": True}
    )
    assert response.status_code == 200
    data = response.json()
    ...
    # Unmute
    response = client.post(
        "/chat-mute/global", headers=auth_headers_user1, json={"muted": False}
    )
    assert response.status_code == 200
...
def test_mute_trivia_live_chat(auth_headers_user1, test_user1, test_db):
    ...
    response = client.post(
        "/chat-mute/trivia-live", headers=auth_headers_user1, json={"muted": True}
    )
    assert response.status_code == 200
    data = response.json()
...
@patch("routers.messaging.global_chat.send_push_for_global_chat_sync")
...
def test_global_chat_muted_no_push(mock_push, auth_headers_user1, test_user1, test_db):
    ...
    # Mute global chat
    client.post("/chat-mute/global", headers=auth_headers_user1, json={"muted": True})
...
def test_global_chat_messages_include_profile_data(
    auth_headers_user1, test_user1, test_db
):
...
@patch("routers.trivia.trivia_live_chat.send_push_for_trivia_live_chat_sync")
def test_trivia_live_chat_send_with_push(
    mock_push, mock_active, auth_headers_user1, test_user1, test_db
):
...
@patch("routers.trivia.trivia_live_chat.is_trivia_live_chat_active")
def test_trivia_live_chat_messages_include_profile_data(
    mock_active, auth_headers_user1, test_user1, test_db
):
    ...
    # Get messages
    response = client.get(
        "/trivia-live-chat/messages?limit=10", headers=auth_headers_user1
    )
    assert response.status_code == 200
    data = response.json()
    assert "messages" in data
...
@patch("routers.messaging.private_chat.send_push_if_needed_sync")
def test_private_chat_send_with_mute_check(
    mock_push, auth_headers_user1, auth_headers_user2, test_user1, test_user2, test_db
):
...
@patch("routers.messaging.private_chat.send_push_if_needed_sync")
def test_private_chat_conversations_include_profile_data(
    mock_push, auth_headers_user1, auth_headers_user2, test_user1, test_user2, test_db
):
...
def test_private_chat_messages_include_profile_data(
    auth_headers_user1, auth_headers_user2, test_user1, test_user2, test_db
):
