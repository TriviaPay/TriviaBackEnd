...
def _make_client(test_db, user, monkeypatch):
    ...
    monkeypatch.setattr(group_members_router, "publish_group_message", lambda *args, **kwargs: None)
...
def test_list_add_and_remove_members(test_db, current_user, peer_user, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        response = client.post(f"/groups/{group.id}/members", json={"user_ids": [peer_user.account_id]})
        assert response.status_code == 200
        ...
        response = client.delete(f"/groups/{group.id}/members/{current_user.account_id}")
        assert response.status_code == 403


def test_add_member_respects_ban_and_capacity(test_db, current_user, peer_user, monkeypatch):
    app = _make_client(test_db, current_user, monkeypatch)
    ...
    test_db.commit()
    test_db.add(GroupBan(group_id=group.id, user_id=peer_user.account_id, banned_by=current_user.account_id))
    test_db.commit()
...
def test_promote_demote_ban_unban_and_leave(test_db, current_user, peer_user, monkeypatch):
    owner_app = _make_client(test_db, current_user, monkeypatch)
    member_app = _make_client(test_db, peer_user, monkeypatch)
    group = _create_group(test_db, current_user.account_id)

    test_db.add(GroupParticipant(group_id=group.id, user_id=peer_user.account_id, role="member"))
    group.participant_count = 2
    test_db.commit()

    with TestClient(owner_app) as client:
        response = client.post(f"/groups/{group.id}/promote", json={"user_id": peer_user.account_id})
        assert response.status_code == 200

        response = client.post(f"/groups/{group.id}/demote", json={"user_id": peer_user.account_id})
        assert response.status_code == 200

        response = client.post(f"/groups/{group.id}/ban", json={"user_id": peer_user.account_id, "reason": "spam"})
        assert response.status_code == 200
        participant = test_db.query(GroupParticipant).filter(
...
def test_mute_group(test_db, current_user, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        response = client.post(f"/groups/{group.id}/mute", json={"mute_until": mute_until})
        assert response.status_code == 200
