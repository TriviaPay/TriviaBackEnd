...
from routers.messaging.global_chat import router as global_chat_router
from routers.messaging.private_chat import router as private_chat_router
from routers.notifications.onesignal import router as onesignal_router
from routers.notifications.pusher_auth import router as pusher_auth_router
from routers.trivia.trivia_live_chat import router as trivia_live_chat_router
...
class TestGlobalChat:
    ...
    def test_send_global_message(self, test_db, auth_override_user1):
        ...
        with patch(
            "routers.messaging.global_chat.publish_chat_message_sync"
        ) as mock_pusher:
            ...
            message = (
                test_db.query(GlobalChatMessage)
                .filter(GlobalChatMessage.id == data["message_id"])
                .first()
            )
    ...
    def test_send_global_message_idempotency(self, test_db, auth_override_user1):
        ...
        response1 = client.post(
            "/global-chat/send",
            json={"message": "First message", "client_message_id": "duplicate_test"},
        )
        ...
        response2 = client.post(
            "/global-chat/send",
            json={"message": "Second message", "client_message_id": "duplicate_test"},
        )
        ...
        messages = (
            test_db.query(GlobalChatMessage)
            .filter(GlobalChatMessage.client_message_id == "duplicate_test")
            .all()
        )
    ...
    def test_get_global_messages(self, test_db, auth_override_user1, test_user2):
        ...
        for i in range(5):
            msg = GlobalChatMessage(
                user_id=(
                    test_user2.account_id
                    if i % 2 == 0
                    else auth_override_user1.account_id
                ),
    ...
    def test_global_chat_rate_limiting(self, test_db, auth_override_user1):
        ...
        with patch(
            "routers.messaging.global_chat.GLOBAL_CHAT_MAX_MESSAGES_PER_MINUTE", 2
        ):
            # Send 2 messages (should succeed)
            for i in range(2):
                response = client.post(
                    "/global-chat/send", json={"message": f"Message {i}"}
                )
                assert response.status_code == 200

            # Third message should be rate limited
            response = client.post(
                "/global-chat/send", json={"message": "Rate limited message"}
            )
...
class TestPrivateChat:
    ...
    def test_send_private_message_creates_conversation(
        self, test_db, auth_override_user1, test_user2
    ):
        """Test sending first message creates conversation"""
        with patch("routers.messaging.private_chat.publish_chat_message_sync"), patch(
            "routers.messaging.private_chat.send_push_if_needed_sync"
        ):
            ...
            conversation = (
                test_db.query(PrivateChatConversation)
                .filter(PrivateChatConversation.id == data["conversation_id"])
                .first()
            )
    ...
    def test_send_private_message_auto_accepts(
        self, test_db, auth_override_user1, test_user2
    ):
    ...
    def test_accept_reject_chat_request(self, test_db, auth_override_user1, test_user2):
        ...
        # User2 accepts
        app.dependency_overrides[get_current_user] = lambda: test_user2
        with patch("routers.messaging.private_chat.publish_chat_message_sync"):
            response = client.post(
                "/private-chat/accept-reject",
                json={"conversation_id": conversation_id, "action": "accept"},
            )
    ...
    def test_reject_chat_request(self, test_db, auth_override_user1, test_user2):
        ...
        with patch("routers.messaging.private_chat.publish_chat_message_sync"):
            response = client.post(
                "/private-chat/accept-reject",
                json={"conversation_id": conversation_id, "action": "reject"},
            )
    ...
    def test_list_private_conversations(
        self, test_db, auth_override_user1, test_user2, test_user3
    ):
    ...
    def test_private_chat_blocking(self, test_db, auth_override_user1, test_user2):
        ...
        # Create block
        block = Block(
            blocker_id=test_user2.account_id, blocked_id=auth_override_user1.account_id
        )
        test_db.add(block)
        test_db.commit()
        ...
        response = client.post(
            "/private-chat/send",
            json={"recipient_id": test_user2.account_id, "message": "Blocked message"},
        )
        assert response.status_code == 403
...
class TestTriviaLiveChat:
    ...
    @patch("routers.trivia.trivia_live_chat.is_trivia_live_chat_active")
    def test_send_trivia_live_message(self, mock_active, test_db, auth_override_user1):
        ...
        with patch(
            "routers.trivia.trivia_live_chat.publish_chat_message_sync"
        ) as mock_pusher:
            response = client.post(
            ...
            trivia_msg = (
                ...
                .filter(TriviaLiveChatMessage.id == data["message_id"])
                .first()
            )
            ...
            global_msg = (
                test_db.query(GlobalChatMessage)
                .filter(GlobalChatMessage.message == "Trivia chat message!")
                .first()
            )
    ...
    def test_trivia_live_chat_inactive(self, mock_active, test_db, auth_override_user1):
        ...
        response = client.post(
            "/trivia-live-chat/send", json={"message": "Should fail"}
        )
...
class TestOneSignal:
    ...
    def test_register_player(self, test_db, auth_override_user1):
        ...
        response = client.post(
            "/onesignal/register",
            json={"player_id": "test_player_123", "platform": "ios"},
        )
        ...
        player = (
            test_db.query(OneSignalPlayer)
            .filter(OneSignalPlayer.player_id == "test_player_123")
            .first()
        )
    ...
    def test_update_player(self, test_db, auth_override_user1):
        ...
        response = client.post(
            "/onesignal/register",
            json={"player_id": "test_player_456", "platform": "ios"},
        )
...
class TestPusherAuth:
    ...
    def test_pusher_auth_private_channel(
        self, test_db, auth_override_user1, test_user2
    ):
        ...
        with patch(
            "routers.notifications.pusher_auth.get_pusher_client"
        ) as mock_pusher:
    ...
    def test_pusher_auth_unauthorized_channel(
        self, test_db, auth_override_user1, test_user2, test_user3
    ):
    ...
    def test_pusher_auth_public_channel(self, auth_override_user1):
        ...
        response = client.post(
            "/pusher/auth", data={"socket_id": "123.456", "channel_name": "global-chat"}
        )
...
class TestChatIntegration:
    ...
    def test_trivia_message_separate_from_global(self, test_db, auth_override_user1):
        ...
        with patch(
            ...
            return_value=True,
        ), patch("routers.trivia.trivia_live_chat.publish_chat_message_sync"):

            response = client.post(
                "/trivia-live-chat/send", json={"message": "Trivia message"}
            )
            ...
            trivia_msg_in_global = next(
                (m for m in messages if m.get("id") == trivia_msg_id), None
            )
    ...
    def test_private_chat_unread_count(self, test_db, auth_override_user1, test_user2):
        ...
        # Mark as read
        last_msg = (
            test_db.query(PrivateChatMessage)
            .filter(PrivateChatMessage.conversation_id == conv.id)
