...
from models import LiveChatLike, LiveChatMessage, LiveChatSession, LiveChatViewer, User
from routers.live_chat import (
    ConnectionManager,
    get_or_create_active_session,
    is_chat_window_active,
)
...
class TestLiveChatModels:
    ...
    def test_live_chat_viewer_creation(self, db_session, test_user):
        ...
        viewer = LiveChatViewer(
            session_id=session.id, user_id=test_user.account_id, is_active=True
        )
...
class TestLiveChatFunctions:
    ...
    @patch("routers.live_chat.get_next_draw_time")
    def test_get_or_create_active_session_new(
        self, mock_get_next_draw_time, db_session
    ):
...
class TestLiveChatEndpoints:
    ...
    def test_get_chat_status_enabled_inactive(self):
        ...
        with patch("routers.live_chat.LIVE_CHAT_ENABLED", True), patch(
            "routers.live_chat.is_chat_window_active", return_value=False
        ):
    ...
    def test_get_chat_status_enabled_active(self, test_user_token):
        ...
        with patch("routers.live_chat.LIVE_CHAT_ENABLED", True), patch(
            "routers.live_chat.is_chat_window_active", return_value=True
...
def test_user(db_session):
    """Create a test user"""
    user = User(
        email="test@example.com", username="testuser", descope_user_id="test_descope_id"
    )
    db_session.add(user)
    db_session.commit()
