...
from routers.messaging import private_chat as private_chat_router
...
def _setup_app(test_db, user, monkeypatch):
    ...
    monkeypatch.setattr(
        private_chat_router, "check_burst_limit", AsyncMock(return_value=True)
    )
    monkeypatch.setattr(
        private_chat_router, "check_rate_limit", AsyncMock(return_value=True)
    )
    monkeypatch.setattr(
        private_chat_router, "enqueue_chat_event", AsyncMock(return_value=True)
    )
    monkeypatch.setattr(
        private_chat_router, "should_emit_typing_event", AsyncMock(return_value=True)
    )
    monkeypatch.setattr(
        private_chat_router, "clear_typing_event", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(
        private_chat_router, "publish_chat_message_sync", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        private_chat_router, "send_push_if_needed_sync", lambda *args, **kwargs: None
    )
...
def test_messages_mark_read_delivered_and_typing(test_db, user1, user2, monkeypatch):
    ...
    with TestClient(app_user1) as client_user1, TestClient(app_user2) as client_user2:
        ...
        response = client_user2.post(
            f"/private-chat/messages/{message_id}/mark-delivered"
        )
        assert response.status_code == 200

        response = client_user2.post(
            f"/private-chat/conversations/{conversation_id}/mark-read"
        )
        assert response.status_code == 200

        response = client_user1.post(
            f"/private-chat/conversations/{conversation_id}/typing"
        )
        assert response.status_code == 200
        response = client_user1.post(
            f"/private-chat/conversations/{conversation_id}/typing-stop"
        )
        assert response.status_code == 200
...
def test_block_unblock_and_presence_creation(test_db, user1, user2, monkeypatch):
    ...
    with TestClient(app_user1) as client:
        response = client.post(
            "/private-chat/block", json={"blocked_user_id": user2.account_id}
        )
        assert response.status_code == 200
        ...
        assert (
            ...
            .filter(UserPresence.user_id == user2.account_id)
            .count()
            == 1
        )
