...
from routers.messaging import status_metrics as status_metrics_router
...
def test_metrics_admin_required(test_db, admin_user, monkeypatch):
    ...
    non_admin = (
        test_db.query(User).filter(User.account_id != admin_user.account_id).first()
    )
...
def test_metrics_counts_and_average(client, test_db, admin_user):
    ...
    test_db.add_all(
        [
            StatusAudience(
                post_id=post_active.id, viewer_user_id=admin_user.account_id
            ),
            StatusAudience(
                post_id=post_active.id, viewer_user_id=admin_user.account_id + 1
            ),
            StatusAudience(
                post_id=post_expired.id, viewer_user_id=admin_user.account_id
            ),
        ]
    )
    test_db.add(
        StatusView(
            post_id=post_active.id, viewer_user_id=admin_user.account_id, viewed_at=now
        )
    )
    test_db.commit()
