...
class TestStripeConnectAccountLink:
    ...
    @pytest.mark.asyncio
    async def test_create_account_link_new_account(self, mock_user_no_stripe):
        """Test creating account link for new user"""
        from app.routers.payments.stripe_connect import (
            create_account_link,
            create_or_get_connect_account,
        )
        ...
        with patch(
            "app.routers.payments.stripe_connect.create_or_get_connect_account",
            return_value=STRIPE_TEST_ACCOUNT_ID_NEW,
        ), patch(
            "app.routers.payments.stripe_connect.create_account_link",
            return_value={"url": STRIPE_TEST_ONBOARDING_URL, "expires_at": 1234567890},
        ):
    ...
    @pytest.mark.asyncio
    async def test_create_account_link_existing_account(self, mock_user):
        ...
        with patch(
            "app.routers.payments.stripe_connect.create_or_get_connect_account",
            return_value=STRIPE_TEST_ACCOUNT_ID,
        ), patch(
            "app.routers.payments.stripe_connect.create_account_link",
            return_value={"url": STRIPE_TEST_REFRESH_URL, "expires_at": 1234567890},
        ):
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.post(
                    "/api/v1/stripe/connect/create-account-link",
    ...
    @pytest.mark.asyncio
    async def test_create_account_link_with_custom_urls(self, mock_user_no_stripe):
        ...
        with patch(
            ...
            "app.routers.payments.stripe_connect.create_account_link", mock_create_link
        ):
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.post(
                    "/api/v1/stripe/connect/create-account-link",
    ...
    @pytest.mark.asyncio
    async def test_create_account_link_stripe_error(self, mock_user_no_stripe):
        ...
        with patch(
            "app.routers.payments.stripe_connect.create_or_get_connect_account",
            side_effect=StripeError("Stripe API error: Rate limit exceeded"),
        ):
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.post(
                    "/api/v1/stripe/connect/create-account-link",
...
class TestRefreshAccountLink:
    ...
    @pytest.mark.asyncio
    async def test_refresh_account_link_success(self, mock_user):
    ...
    @pytest.mark.asyncio
    async def test_refresh_account_link_no_account(self, mock_user_no_stripe):
...
class TestPublishableKey:
    ...
    async def test_get_publishable_key_success(self):
        ...
        with patch(
            "app.routers.payments.stripe_connect.get_publishable_key",
            return_value=STRIPE_TEST_PUBLISHABLE_KEY,
        ):
            async with AsyncClient(app=app, base_url="http://test") as ac:
    ...
    async def test_get_publishable_key_not_configured(self):
        ...
        with patch(
            "app.routers.payments.stripe_connect.get_publishable_key", return_value=None
        ):
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get("/api/v1/stripe/connect/publishable-key")
