    ...
    try:
        ...
        for package in one_time_packages:
            ...
            purchases = (
                ...
                .filter(UserGemPurchase.package_id == package.id)
                .all()
            )
            ...
            if missing_purchase_users:
                ...
                logger.info(
                    f"Found {len(missing_purchase_users)} users who need their eligibility restored for package {package.id}"
                )
        ...
        total_deleted = (
            db.query(UserGemPurchase)
            .filter(UserGemPurchase.package_id.in_([p.id for p in one_time_packages]))
...
if __name__ == "__main__":
    ...
    if success:
        print(
            "One-time purchase records have been reset successfully. All users can now purchase one-time offers again."
        )
    else:
