    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if not use_sandbox and result.get("status") == APPLE_STATUS_SANDBOX_RECEIPT:
                logger.info("Received sandbox receipt in production, retrying with sandbox endpoint")
                async with httpx.AsyncClient(timeout=30.0) as sandbox_client:
                    sandbox_response = await sandbox_client.post(APPLE_SANDBOX_URL, json=payload)
...
async def extract_latest_apple_transaction(
    ...
    if status_code != APPLE_STATUS_OK:
        ...
        error_msg = error_messages.get(status_code, f"Receipt verification failed with status {status_code}")
        raise HTTPException(
    ...
    matching_transactions = [
        tx for tx in latest_receipt_info
        if tx.get("product_id") == expected_product_id
    ]
    ...
    latest_transaction = max(
        matching_transactions,
        key=lambda tx: int(tx.get("purchase_date_ms", 0))
    )
    
    # Use original_transaction_id if available (for subscriptions), otherwise transaction_id
    transaction_id = latest_transaction.get("original_transaction_id") or latest_transaction.get("transaction_id")
...
async def process_apple_iap(
    ...
    try:
        transaction_id, confirmed_product_id, purchase_date_ms = await extract_latest_apple_transaction(
    ...
    if existing_receipt:
        if existing_receipt.status in ('verified', 'consumed'):
            ...
            logger.warning(f"Apple IAP transaction {transaction_id} previously failed, retrying")
