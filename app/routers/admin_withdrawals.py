...
async def list_withdrawals(
    ...
    stmt = select(WithdrawalRequest, User).join(
        User, WithdrawalRequest.user_id == User.account_id
    ).where(
    ...
    stmt = stmt.order_by(desc(WithdrawalRequest.requested_at)).limit(limit).offset(offset)
...
async def approve_withdrawal(
    ...
    """
    # Lock withdrawal row
    stmt = select(WithdrawalRequest).where(WithdrawalRequest.id == withdrawal_id).with_for_update()
    result = await db.execute(stmt)
    ...
    # Lock user row
    user_stmt = select(User).where(User.account_id == withdrawal.user_id).with_for_update()
