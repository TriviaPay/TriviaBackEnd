...
async def stripe_webhook(
    ...
    logger.info(
        f"Received Stripe webhook: {event_type} (event_id: {event_id}, livemode: {livemode})"
    )
    ...
    try:
        ...
        if event_type == "account.updated":
            ...
            if user:
                ...
                await db.commit()
                logger.info(
                    f"Updated Connect account status for user {user.account_id}: charges={charges_enabled}, payouts={payouts_enabled}, details={details_submitted}"
                )
            else:
            ...
            if withdrawal:
                if withdrawal.status != "paid":
                    ...
                    await db.commit()
                    logger.info(
                        f"Updated withdrawal {withdrawal.id} to paid status for payout {transfer_id}"
                    )
                else:
                    logger.info(f"Withdrawal {withdrawal.id} already marked as paid")
            else:
                logger.warning(
                    f"Withdrawal request not found for payout/transfer {transfer_id}"
                )
            ...
            if withdrawal:
                if withdrawal.status != "failed":
                    ...
                    try:
                        ...
                        withdrawal.admin_notes = (
                            f"Payout failed via webhook: {transfer_id}"
                        )

                        await db.commit()
                        logger.info(
                            f"Refunded withdrawal {withdrawal.id}: {refund_amount} {withdrawal.currency} to user {withdrawal.user_id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to refund withdrawal {withdrawal.id}: {str(e)}"
                        )
                        await db.rollback()
                        raise
                else:
                ...
                logger.warning(
                    f"Withdrawal request not found for failed payout/transfer {transfer_id}"
                )
            ...
            if not account_id_str:
                logger.warning(
                    f"PaymentIntent {payment_intent_id} missing account_id in metadata, skipping"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Invalid account_id in PaymentIntent {payment_intent_id}: {account_id_str}"
                )
            ...
            if not user:
                logger.warning(
                    f"User {account_id} not found for PaymentIntent {payment_intent_id}, skipping"
                )
                # Still record event for idempotency
                transaction = WalletTransaction(
                    user_id=0,
            ...
            if topup_type == "wallet_topup":
                ...
                kind = "deposit"
                logger.warning(
                    f"Unknown topup_type '{topup_type}' for PaymentIntent {payment_intent_id}, using 'deposit'"
                )
            ...
            logger.info(
                f"PaymentIntent {payment_intent_id} amount capturable updated: capturable={amount_capturable}, received={amount_received}"
            )
            ...
            logger.info(
                f"Subscription {subscription_id} trial ending for customer {customer_id} at {trial_end}"
            )
            ...
            logger.info(
                f"Subscription {subscription_id} {event_type}: status={subscription_status}, customer={customer_id}"
            )
