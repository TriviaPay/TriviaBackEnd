...
def check_five_dollar_subscription():
    ...
    try:
        ...
        active_subs = db.query(UserSubscription).join(SubscriptionPlan).filter(
            (SubscriptionPlan.unit_amount_minor == 500) | 
            (SubscriptionPlan.price_usd == 5.0),
        ...
        for sub in active_subs:
            ...
            print(f"   User: {user.username if user else sub.user_id} (ID: {sub.user_id})")
            print(f"   Status: {sub.status}")
            print(f"   Period End: {sub.current_period_end}")
            print()
        
        # Check all $5 subscriptions (including inactive)
        all_subs = db.query(UserSubscription).join(SubscriptionPlan).filter(
            (SubscriptionPlan.unit_amount_minor == 500) | 
            (SubscriptionPlan.price_usd == 5.0)
        ).all()
