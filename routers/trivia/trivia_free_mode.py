...
from routers.dependencies import get_current_user_or_guest, require_non_guest
...
from .service import (
    ...
    guest_ad_bonus as service_guest_ad_bonus,
...
async def get_free_mode_questions(
    user=Depends(get_current_user_or_guest), db: Session = Depends(get_db)
):
...
async def double_gems_after_win(
    draw_date: Optional[str] = None,
    user=Depends(require_non_guest),
    db: Session = Depends(get_db),
):
...
@router.post("/guest-ad-bonus")
async def claim_guest_ad_bonus(
    user=Depends(get_current_user_or_guest),
    db: Session = Depends(get_db),
):
    return service_guest_ad_bonus(db, user=user)
