    ...
    if not daily_question:
        ...
        try:
            ...
            daily_question = (
                db.query(TriviaQuestionsSilverModeDaily)
                .options(joinedload(TriviaQuestionsSilverModeDaily.question))
                .filter(TriviaQuestionsSilverModeDaily.id == daily_question.id)
                .first()
            )
...
async def submit_silver_mode_answer(
    ...
    question = (
        db.query(TriviaQuestionsSilverMode)
        .filter(TriviaQuestionsSilverMode.id == request.question_id)
        .first()
    )
...
async def get_silver_mode_status(
    ...
    is_winner = (
        ...
        .filter(
            ...
            TriviaSilverModeWinners.draw_date == winner_draw_date,
        )
        .first()
        is not None
    )
...
async def get_silver_mode_leaderboard(
    ...
    leaderboard_entries = (
        ...
        .filter(TriviaSilverModeLeaderboard.draw_date == target_date)
        .order_by(
