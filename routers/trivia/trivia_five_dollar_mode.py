...
async def get_bronze_mode_question(
    ...
    if not daily_question:
        logger.info(
            f"No question allocated for {target_date}, attempting auto-allocation..."
        )
        try:
            ...
            if not selected_question:
                selected_question = (
                    db.query(TriviaQuestionsBronzeMode).order_by(func.random()).first()
                )
                if not selected_question:
                    raise HTTPException(
                        status_code=404,
            ...
            daily_question = (
                db.query(TriviaQuestionsBronzeModeDaily)
                .options(joinedload(TriviaQuestionsBronzeModeDaily.question))
                .filter(TriviaQuestionsBronzeModeDaily.id == daily_question.id)
                .first()
            )

            logger.info(
                f"Auto-allocated question {selected_question.id} for {target_date}"
            )
        except Exception as e:
            db.rollback()
    ...
    draw_time = now.replace(
        hour=draw_time_hour, minute=draw_time_minute, second=0, microsecond=0
    )
    ...
    question_data = {
        ...
        "time_until_close_seconds": (
            int(time_until_close) if time_until_close > 0 else 0
        ),
    }
...
async def submit_bronze_mode_answer(
    ...
    if existing_attempt and existing_attempt.submitted_at:
        raise HTTPException(
            status_code=400, detail="You have already submitted an answer for today"
        )

    # Get the question
    question = (
        db.query(TriviaQuestionsBronzeMode)
        .filter(TriviaQuestionsBronzeMode.id == request.question_id)
        .first()
    )
    ...
    if not daily_q:
        raise HTTPException(status_code=400, detail="Question not available for today")
    ...
    if now >= draw_time:
        raise HTTPException(status_code=400, detail="Question submission is closed")
...
async def get_bronze_mode_status(
    ...
    return {
        ...
        "has_submitted": user_attempt is not None
        and user_attempt.submitted_at is not None,
        "submitted_at": (
            user_attempt.submitted_at.isoformat()
            if user_attempt and user_attempt.submitted_at
            else None
...
async def get_bronze_mode_leaderboard(
    ...
    leaderboard_entries = (
        ...
        .filter(TriviaBronzeModeLeaderboard.draw_date == target_date)
        .order_by(
