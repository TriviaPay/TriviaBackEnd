...
class SendMessageRequest(BaseModel):
    message: str = Field(
        ..., min_length=1, max_length=TRIVIA_LIVE_CHAT_MAX_MESSAGE_LENGTH
    )
...
def is_trivia_live_chat_active() -> bool:
    ...
    try:
        ...
        logger.debug(
            f"Trivia live chat - Next window: {next_chat_start} to {next_chat_end}"
        )
        logger.debug(
            f"Trivia live chat - Prev window: {prev_chat_start} to {prev_chat_end}"
        )
        logger.debug(f"Trivia live chat - Current time: {now}")
        logger.debug(
            f"Trivia live chat - In next window: {in_next_window}, In prev window: {in_prev_window}"
        )
...
async def send_trivia_live_message(
    ...
    if request.client_message_id:
        ...
        if existing_message:
            logger.debug(
                f"Duplicate trivia live chat message detected: {request.client_message_id}"
            )
            return {
    ...
    if minute_allowed is None:
        ...
        recent_messages = (
            ...
            .order_by(TriviaLiveChatMessage.created_at.desc())
            .limit(TRIVIA_LIVE_CHAT_MAX_MESSAGES_PER_MINUTE)
            .all()
        )
...
async def get_trivia_live_messages(
    ...
    if reply_message_ids:
        replied_msgs = (
            db.query(TriviaLiveChatMessage)
            .options(joinedload(TriviaLiveChatMessage.user))
            .filter(TriviaLiveChatMessage.id.in_(list(reply_message_ids)))
            .all()
        )
    ...
    for msg in reversed(messages):
        ...
        if msg.reply_to_message_id:
            ...
            if replied_msg:
                replied_sender_profile = profile_cache.get(
                    replied_msg.user_id,
                    {
                        "profile_pic_url": None,
                        "avatar_url": None,
                ...
                reply_info = {
                    ...
                    "sender_level_progress": replied_sender_profile.get(
                        "level_progress", "0/100"
                    ),
                }
...
async def debug_trivia_live_chat_config(current_user: User = Depends(get_current_user)):
    """Debug endpoint to check configuration values"""
    return {
        "TRIVIA_LIVE_CHAT_PRE_HOURS_env": os.getenv(
            "TRIVIA_LIVE_CHAT_PRE_HOURS", "NOT SET"
        ),
        "TRIVIA_LIVE_CHAT_POST_HOURS_env": os.getenv(
            "TRIVIA_LIVE_CHAT_POST_HOURS", "NOT SET"
        ),
...
async def like_trivia_live_chat(
    ...
    if not is_trivia_live_chat_active():
        raise HTTPException(
            status_code=403, detail="Trivia live chat is not currently active"
        )
