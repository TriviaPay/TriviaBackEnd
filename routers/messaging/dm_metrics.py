...
def get_dm_metrics(
    ...
    total_sse_connections = sum(
        len(sessions) for sessions in _active_dm_sse_connections.values()
    )
    ...
    old_prekeys = (
        ...
        .filter(E2EEKeyBundle.updated_at < old_prekey_cutoff)
        .count()
    )
    ...
    message_counts = db.query(
        ...
        .label("today"),
        func.count(DMMessage.id)
        .filter(DMMessage.created_at >= now - timedelta(hours=1))
        .label("last_hour"),
    ).one()
