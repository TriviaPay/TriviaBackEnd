...
def _get_cached_metrics(now_ts: float) -> Optional[Dict[str, Any]]:
    ...
    ts = _metrics_cache.get("ts") or 0.0
...
def get_group_metrics(
    ...
    avg_size = (
        db.query(func.avg(participant_counts_subq.c.participant_count)).scalar() or 0
    )
    ...
    message_counts = db.query(
        ...
        .label("today"),
        func.count(GroupMessage.id)
        .filter(GroupMessage.created_at >= now - timedelta(hours=1))
        .label("last_hour"),
    ).one()
    ...
    groups_with_epoch_changes = (
        db.query(Group).filter(Group.updated_at >= yesterday).count()
    )
