...
from config import GROUP_INVITE_EXPIRY_HOURS, GROUP_MAX_PARTICIPANTS, GROUPS_ENABLED
from db import get_db
from models import Group, GroupBan, GroupInvite, GroupParticipant, User
from routers.dependencies import get_current_user
from routers.messaging.group_members import check_group_role, increment_group_epoch
...
class JoinGroupRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"code": "ABC123XYZ"}}
...
async def list_invites(
    ...
    active_invites = (
        ...
        .filter(
            ...
            or_(
                GroupInvite.max_uses.is_(None), GroupInvite.uses < GroupInvite.max_uses
            ),
        )
        .all()
    )
...
async def join_group(
    ...
    group = (
        db.query(Group).filter(Group.id == invite.group_id).with_for_update().first()
    )
    if not group:
        raise HTTPException(status_code=404, detail="Group not found")
    ...
    ban = (
        db.query(GroupBan)
        .filter(
            GroupBan.group_id == group.id, GroupBan.user_id == current_user.account_id
        )
        .first()
    )
    ...
    participant_count = (
        db.query(GroupParticipant)
        .filter(
            GroupParticipant.group_id == group.id, GroupParticipant.is_banned == False
        )
        .count()
    )
