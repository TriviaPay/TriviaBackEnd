...
from models import E2EEDevice, Group, GroupBan, GroupParticipant, User
...
class AddMembersRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"user_ids": [1142961859, 9876543210]}}
...
class PromoteRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"user_id": 1142961859}}
...
class BanRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"user_id": 1142961859, "reason": "Violation of group rules"}
        }


class MuteRequest(BaseModel):
    mute_until: Optional[datetime] = Field(
        None, example="2025-11-10T16:00:00Z"
    )  # None = unmute

    class Config:
        json_schema_extra = {"example": {"mute_until": "2025-11-10T16:00:00Z"}}


def check_group_role(
    db: Session, group_id: uuid.UUID, user_id: int, required_roles: List[str]
) -> GroupParticipant:
    """Check if user has required role in group."""
    participant = (
        db.query(GroupParticipant)
        .filter(
            GroupParticipant.group_id == group_id, GroupParticipant.user_id == user_id
        )
        .first()
    )
...
def _get_group_member_count(db: Session, group: Group, group_id: uuid.UUID) -> int:
    ...
    return (
        db.query(GroupParticipant)
        .filter(
            GroupParticipant.group_id == group_id, GroupParticipant.is_banned == False
        )
        .count()
    )
...
async def list_members(
    ...
    participants = (
        db.query(GroupParticipant)
        .filter(
            GroupParticipant.group_id == group_uuid, GroupParticipant.is_banned == False
        )
        .all()
    )
    ...
    for user_id in user_ids:
        ...
        if existing:
            ...
            new_participants.append(
                GroupParticipant(
                    group_id=group_uuid, user_id=user_id, role="member", joined_at=now
                )
            )
            added_users.append(user_id)
            active_delta += 1
...
async def remove_member(
    ...
    target_participant = (
        db.query(GroupParticipant)
        .filter(
            GroupParticipant.group_id == group_uuid, GroupParticipant.user_id == user_id
        )
        .first()
    )
