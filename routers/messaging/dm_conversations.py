...
from models import Block, DMConversation, DMMessage, DMParticipant, User
...
class CreateConversationRequest(BaseModel):
    peer_user_id: int = Field(
        ..., description="User ID of the peer user", example=1142961859
    )

    class Config:
        json_schema_extra = {"example": {"peer_user_id": 1142961859}}
...
async def create_or_find_conversation(
    ...
    if check_blocked(db, current_user.account_id, request.peer_user_id):
        raise HTTPException(
            status_code=403, detail="Cannot create conversation with blocked user"
        )
    ...
    if hasattr(DMConversation, "pair_key"):
        existing = (
            db.query(DMConversation).filter(DMConversation.pair_key == pair_key).first()
        )
    ...
    if existing:
        ...
        participants = (
            ...
            .filter(DMParticipant.conversation_id == existing.id)
            .all()
        )
    ...
    if hasattr(DMConversation, "pair_key"):
        new_conversation = DMConversation(
            id=uuid.uuid4(), created_at=datetime.utcnow(), pair_key=pair_key
        )
    else:
        new_conversation = DMConversation(id=uuid.uuid4(), created_at=datetime.utcnow())
    ...
    try:
        ...
        if hasattr(DMConversation, "pair_key"):
            existing = (
                db.query(DMConversation)
                .filter(DMConversation.pair_key == pair_key)
                .first()
            )
        else:
            existing = (
    ...
    device_rows = (
        ...
        .filter(E2EEDevice.user_id.in_(user_ids), E2EEDevice.status == "active")
        .all()
    )
    device_map: Dict[int, List[str]] = {user_id: [] for user_id in user_ids}
    ...
    logger.info(
        f"Created conversation {new_conversation.id} between users {current_user.account_id} and {request.peer_user_id}"
    )
    ...
    return {
        ...
        "participants": [
            ...
            {"user_id": request.peer_user_id, "device_ids": peer_device_ids},
        ],
    }
...
async def list_conversations(
    ...
    try:
        ...
        conversations = (
            db.query(DMConversation)
            .join(DMParticipant, DMConversation.id == DMParticipant.conversation_id)
            .filter(DMParticipant.user_id == current_user.account_id)
            .order_by(
                func.coalesce(
                    DMConversation.last_message_at, DMConversation.created_at
                ).desc(),
...
async def get_conversation(
    ...
    conversation = (
        db.query(DMConversation).filter(DMConversation.id == conv_uuid).first()
    )
    ...
    participants = (
        db.query(DMParticipant).filter(DMParticipant.conversation_id == conv_uuid).all()
    )
