        ...
        if GROUPS_ENABLED:
            try:
                ...
                group_ids = [
                    ...
                    for participant in db.query(GroupParticipant).filter(
                        GroupParticipant.user_id == user_id,
                        GroupParticipant.is_banned == False
                    ).all()
                ]
            except Exception as e:
                logger.warning(f"Group models unavailable, skipping group subscriptions: {e}")
...
async def dm_sse(
    ...
    if not token:
        if token_param and E2EE_DM_SSE_ALLOW_QUERY_TOKEN:
            ...
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Use Authorization header for SSE")
    ...
    try:
        async def event_stream() -> AsyncGenerator[bytes, None]:
            ...
            dm_task = asyncio.create_task(redis_iter.__anext__()) if redis_iter is not None else None
            group_tasks = {
            ...
            try:
                while True:
                    ...
                    if heartbeat in done:
                        ...
                        if (
                            ...
                            now = datetime.utcnow()
                            if (now - last_presence_update).total_seconds() >= PRESENCE_UPDATE_INTERVAL_SECONDS:
                                await run_in_threadpool(_update_presence, user_id, now, None, False)
                                last_presence_update = now
                        ...
                        if now_ts - last_redis_check >= REDIS_RETRY_INTERVAL_SECONDS:
                            ...
                            if redis_available and redis_iter is None:
                                try:
                                    ...
                                    logger.warning(f"Failed to resubscribe to DM channel for user {user_id_hash}: {e}")
                                    redis_iter = None
                            if redis_available and group_ids:
                                for group_id in group_ids:
                                    ...
                                    try:
                                        ...
                                        logger.warning(f"Failed to resubscribe to group {group_id}: {e}")
                            ...
                            for group_id, group_task in list(group_tasks.items()):
                                if group_task is task:
                                    ...
                                    if group_iter is not None:
                                        group_tasks[group_id] = asyncio.create_task(group_iter.__anext__())
                                    break
                ...
                if PRESENCE_ENABLED:
                    await run_in_threadpool(_update_presence, user_id, None, False, False)
