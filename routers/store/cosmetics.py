...
from models import Avatar, Frame, User, UserAvatar, UserFrame
    ...
    for av in avatars:
        ...
        if include_urls and bucket and object_key:
            try:
                ...
                if not signed:
                    logging.warning(
                        f"presign_get returned None for avatar {av.id} with bucket={bucket}, key={object_key}"
                    )
            except Exception as e:
                logging.error(f"Failed to presign avatar {av.id}: {e}", exc_info=True)
        else:
            logging.debug(
                f"Avatar {av.id} missing bucket/object_key: bucket={bucket}, object_key={object_key}"
            )
        out.append(
            AvatarResponse(
                id=av.id,
...
def get_user_avatars(
    ...
    rows = (
        ...
        .filter(UserAvatar.user_id == user.account_id)
...
async def buy_avatar(
    ...
    existing = (
        db.query(UserAvatar)
        .filter(
            UserAvatar.user_id == user.account_id, UserAvatar.avatar_id == avatar_id
        )
        .first()
    )
...
async def select_avatar(
    ...
    if not ownership:
        raise HTTPException(
            status_code=403, detail=f"You don't own the avatar with ID {avatar_id}"
        )
...
def get_all_frames(
    ...
    """
    frames = (
        db.query(Frame).order_by(desc(Frame.created_at)).offset(skip).limit(limit).all()
    )
    ...
    for fr in frames:
        ...
        if (
            include_urls
            and getattr(fr, "bucket", None)
            and getattr(fr, "object_key", None)
        ):
            try:
...
def get_user_frames(
    ...
    rows = (
        ...
        .filter(UserFrame.user_id == user.account_id)
...
async def buy_frame(
    ...
    existing = (
        db.query(UserFrame)
        .filter(UserFrame.user_id == user.account_id, UserFrame.frame_id == frame_id)
        .first()
    )
    ...
    if payment_method == "gems":
        ...
        try:
            ...
            existing = (
                db.query(UserFrame)
                .filter(
                    UserFrame.user_id == user.account_id, UserFrame.frame_id == frame_id
                )
                .first()
            )
            return PurchaseResponse(
...
async def select_frame(
    ...
    if not ownership:
        raise HTTPException(
            status_code=403, detail=f"You don't own the frame with ID {frame_id}"
        )
