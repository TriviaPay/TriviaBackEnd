...
from models import User, Group, GroupParticipant, GroupMessage, GroupDelivery, E2EEDevice
...
from routers.group_members import check_group_role
...
async def get_messages(
    ...
    if before:
        try:
            ...
            before_msg = db.query(GroupMessage).filter(GroupMessage.id == before_uuid).first()
            if before_msg:
                query = query.filter(
    ...
    if after:
        try:
            ...
            after_msg = db.query(GroupMessage).filter(GroupMessage.id == after_uuid).first()
            if after_msg:
                query = query.filter(
    ...
    messages = query.order_by(desc(GroupMessage.created_at), desc(GroupMessage.id)).limit(limit).all()
...
async def send_message(
    ...
    # Check membership
    participant = check_group_role(db, group_uuid, current_user.account_id, ['owner', 'admin', 'member'])
    ...
    if request.client_message_id:
        existing = db.query(GroupMessage).filter(
            GroupMessage.client_message_id == request.client_message_id
        ).first()
        if existing:
            return {
                "id": str(existing.id),
    ...
    if recent_group_messages >= GROUP_BURST_PER_5S:
        ...
        raise HTTPException(
            status_code=429,
            detail=f"Burst rate limit exceeded.",
            headers={
                "X-Retry-After": str(retry_in_seconds),
...
async def mark_delivered(
    ...
    delivery = db.query(GroupDelivery).filter(
