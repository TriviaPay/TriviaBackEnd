...
def get_eligible_participants_free_mode(
    ...
    mode_config = (
        ...
        .filter(TriviaModeConfig.mode_id == "free_mode")
        .first()
    )
    ...
    attempts_subq = (
        ...
        .filter(TriviaUserFreeModeDaily.date == draw_date)
    ...
    rows = (
        ...
        .join(User, User.account_id == attempts_subq.c.account_id)
        .filter(
    ...
    logger.info(
        f"Found {len(eligible_participants)} eligible participants for free mode draw on {draw_date}"
    )
...
def rank_participants_by_completion(
    ...
    """
    return generic_rank_participants_by_completion(
        participants, completion_field="third_question_completed_at"
    )


def calculate_reward_distribution(
    mode_config: TriviaModeConfig, participant_count: int
) -> Dict[str, Any]:
    """
...
def distribute_rewards_to_winners(
    ...
    for winner in winners:
        ...
        if gems_to_award:
            updated = (
                db.query(User)
                .filter(User.account_id == winner["account_id"])
                .update(
                    {User.gems: User.gems + gems_to_award}, synchronize_session=False
                )
            )
            if updated:
        ...
        if leaderboard_entry:
            ...
            leaderboard_entry.completed_at = winner.get(
    ...
    return {"total_winners": len(winners), "total_gems_awarded": total_gems_awarded}
...
def cleanup_old_leaderboard(db: Session, previous_draw_date: date) -> int:
    ...
    deleted_count = (
        db.query(TriviaFreeModeLeaderboard)
        .filter(TriviaFreeModeLeaderboard.draw_date == previous_draw_date)
        .delete()
    )
    ...
    logger.info(
        f"Cleaned up {deleted_count} leaderboard entries for draw date {previous_draw_date}"
    )
