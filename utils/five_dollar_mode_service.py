...
from models import (
    SubscriptionPlan,
    TriviaFiveDollarModeLeaderboard,
    TriviaFiveDollarModeWinners,
    TriviaModeConfig,
    TriviaUserFiveDollarModeDaily,
    User,
    UserSubscription,
)
from utils.mode_rewards_service import rank_participants_by_time
...
def get_eligible_participants_five_dollar_mode(
    ...
    user_attempts = (
        db.query(TriviaUserFiveDollarModeDaily)
        .filter(
            TriviaUserFiveDollarModeDaily.date == draw_date,
            TriviaUserFiveDollarModeDaily.submitted_at.isnot(
                None
            ),  # Must have submitted
        )
        .all()
    ...
    for attempt in user_attempts:
            ...
            .filter(
                and_(
                    ...
                    or_(
                        ...
                        == 5.0,  # Fallback to deprecated field
                    ),
        ...
        if not active_subscription:
            logger.warning(
                f"User {attempt.account_id} submitted but no active $5 subscription"
            )
            continue
    ...
    logger.info(
        f"Found {len(eligible_participants)} eligible participants for $5 mode draw on {draw_date}"
    )
...
def calculate_total_pool_five_dollar_mode(
    ...
    logger.info(
        f"Calculated total pool for $5 mode: ${total_pool:.2f} (participants: {participant_count}, profit_share: {profit_share})"
    )
    return total_pool
...
def distribute_rewards_to_winners_five_dollar_mode(
    ...
    for winner in winners:
        ...
        leaderboard_entry = (
            db.query(TriviaFiveDollarModeLeaderboard)
            .filter(
                and_(
        ...
        if leaderboard_entry:
            ...
            leaderboard_entry.submitted_at = winner.get(
    ...
    return {"total_winners": len(winners), "total_money_awarded": total_money_awarded}


def cleanup_old_leaderboard_five_dollar_mode(
    db: Session, previous_draw_date: date
) -> int:
    """
    ...
    deleted_count = (
        db.query(TriviaFiveDollarModeLeaderboard)
        .filter(TriviaFiveDollarModeLeaderboard.draw_date == previous_draw_date)
        .delete()
    )
    ...
    logger.info(
        f"Cleaned up {deleted_count} leaderboard entries for $5 mode draw date {previous_draw_date}"
    )
