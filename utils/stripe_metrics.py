...
from models import StripeWebhookEvent, WalletLedger, WithdrawalRequest
...
def get_webhook_metrics(db: Session, hours: int = 24) -> Dict:
    ...
    events = (
        ...
        .filter(StripeWebhookEvent.received_at >= since)
        .group_by(StripeWebhookEvent.type, StripeWebhookEvent.status)
        .all()
    )
...
def get_wallet_metrics(db: Session, currency: str = "usd", hours: int = 24) -> Dict:
    ...
    # Total deposits
    deposits = (
        db.query(func.sum(WalletLedger.delta_minor))
        .filter(
    ...
    # Total withdrawals
    withdrawals = (
        db.query(func.sum(func.abs(WalletLedger.delta_minor)))
        .filter(
            WalletLedger.currency == currency,
...
def get_withdrawal_metrics(db: Session, hours: int = 24) -> Dict:
    ...
    instant_fees = (
        db.query(func.sum(WithdrawalRequest.fee_minor))
        .filter(
...
def get_reconciliation_drift(db: Session, currency: str = "usd") -> Optional[Dict]:
    ...
    # Get latest snapshot
    snapshot = (
        db.query(StripeReconciliationSnapshot)
        .filter(StripeReconciliationSnapshot.currency == currency)
        .order_by(StripeReconciliationSnapshot.as_of_date.desc())
        .first()
    )
