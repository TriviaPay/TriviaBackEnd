...
def get_mute_preferences(
    user_id: int, db: Session, create_if_missing: bool = True
) -> ChatMutePreferences:
    """
    ...
    """
    preferences = (
        db.query(ChatMutePreferences)
        .filter(ChatMutePreferences.user_id == user_id)
        .first()
    )
...
def is_user_muted_for_private_chat(
    user_id: int, muted_by_user_id: int, db: Session
) -> bool:
    """
