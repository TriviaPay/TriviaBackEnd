"""
Silver Mode ($10) specific service for eligibility, ranking, and reward distribution.
...
from models import (
    TriviaModeConfig, TriviaUserSilverModeDaily, TriviaSilverModeWinners,
    TriviaSilverModeLeaderboard, User, UserSubscription, SubscriptionPlan
)
...
def get_eligible_participants_silver_mode(
    ...
    user_attempts = db.query(TriviaUserSilverModeDaily).filter(
    ...
    for attempt in user_attempts:
        ...
        # Check both unit_amount_minor (1000 cents = $10) and price_usd (deprecated but may still be used)
        active_subscription = db.query(UserSubscription).join(SubscriptionPlan).filter(
            and_(
                ...
                or_(
                    ...
                    SubscriptionPlan.price_usd == 10.0  # Fallback to deprecated field
                ),
        ...
        if not active_subscription:
            logger.warning(f"User {attempt.account_id} submitted but no active $10 subscription")
            continue
    ...
    logger.info(f"Found {len(eligible_participants)} eligible participants for silver mode draw on {draw_date}")
...
def calculate_total_pool_silver_mode(
    ...
    logger.info(f"Silver mode pool for {draw_date}: {active_subscribers} subscribers * $10 * {profit_share} = ${total_pool:.2f}")
...
def distribute_rewards_to_winners_silver_mode(
    ...
    for i, winner in enumerate(winners):
        ...
        leaderboard_entry = db.query(TriviaSilverModeLeaderboard).filter(
    ...
    logger.info(f"Distributed ${total_distributed:.2f} to {distributed_count} silver mode winners for {draw_date}")
...
def cleanup_old_leaderboard_silver_mode(
    db: Session,
    ...
    """
    deleted_count = db.query(TriviaSilverModeLeaderboard).filter(
        TriviaSilverModeLeaderboard.draw_date == previous_draw_date
    ).delete()
    
    db.commit()
    
    if deleted_count > 0:
        logger.info(f"Cleaned up {deleted_count} old silver mode leaderboard entries for {previous_draw_date}")
