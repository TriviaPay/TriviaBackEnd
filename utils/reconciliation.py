...
from models import StripeReconciliationSnapshot, WalletLedger, UserWalletBalance
import config
...
def calculate_platform_balance(db: Session, currency: str = 'usd') -> Dict[str, int]:
    ...
    available_result = db.query(
        func.sum(
            case(
                (WalletLedger.kind.in_(['deposit', 'refund', 'dispute_release']), WalletLedger.delta_minor),
                else_=0
            )
        )
    ).filter(
        WalletLedger.currency == currency,
        WalletLedger.livemode == False  # Only test mode for now
    ).scalar()
    ...
    pending_result = db.query(
        func.sum(
            case(
                (WalletLedger.kind.in_(['withdraw', 'fee', 'dispute_hold']), func.abs(WalletLedger.delta_minor)),
                else_=0
            )
        )
    ).filter(
        WalletLedger.currency == currency,
        WalletLedger.livemode == False
    ).scalar()
    ...
    return {
        'available_minor': net_available,
        'pending_minor': pending
    }


def reconcile_stripe_balance(db: Session, currency: str = 'usd', as_of_date: Optional[date] = None) -> Dict:
    """
    ...
    try:
        ...
        available_diff = stripe_balance['available_minor'] - platform_balance['available_minor']
        pending_diff = stripe_balance['pending_minor'] - platform_balance['pending_minor']
        
        # Store snapshot
        snapshot = StripeReconciliationSnapshot(
...
def reconcile_all_currencies(db: Session, currencies: List[str] = ['usd']) -> List[Dict]:
    """
