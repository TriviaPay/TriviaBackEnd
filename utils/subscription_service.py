...
def get_modes_access_status(
    ...
    mode_configs = {
        ...
        for config in db.query(TriviaModeConfig).filter(TriviaModeConfig.mode_id.in_(mode_ids)).all()
    }
    ...
    required_minors = {item['required_amount_minor'] for item in required_info if item['required_amount_minor'] > 0}
    required_amounts = {item['required_amount'] for item in required_info if item['required_amount'] > 0}

    active_map = {}
    latest_map = {}
    if required_minors or required_amounts:
        subscriptions = db.query(UserSubscription, SubscriptionPlan).join(SubscriptionPlan).filter(
            and_(
                UserSubscription.user_id == user.account_id,
                or_(
                    SubscriptionPlan.unit_amount_minor.in_(list(required_minors) or [0]),
        ...
        for sub, plan in subscriptions:
            for info in required_info:
                ...
                matches = (
                    (plan.unit_amount_minor == required_minor)
                    or (plan.price_usd == required_amount)
                )
                if not matches:
                    continue
        ...
        if inactive_sub:
            results[mode_id] = {
                ...
                'subscription_details': {
                    ...
                    'current_period_end': inactive_sub.current_period_end.isoformat() if inactive_sub.current_period_end else None,
                    'amount': required_amount
                },
