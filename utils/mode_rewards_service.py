...
def calculate_harmonic_sum_rewards(
    participant_count: int, total_pool: float
) -> List[float]:
    """
...
def calculate_reward_distribution(
    ...
    if distribution_method == "harmonic_sum":
        ...
        reward_amounts = calculate_tiered_rewards(
            participant_count, tiered_config, shares, total_pool
        )
        ...
        shares = reward_config.get("shares", [1.0])
        reward_amounts = calculate_fixed_rewards(
            fixed_winner_count, participant_count, shares, total_pool
        )
        winner_count = len(reward_amounts)

    else:
        logger.warning(
            f"Unknown distribution method: {distribution_method}, defaulting to tiered"
        )
        tiered_config = reward_config.get("tiered_config", {"default": 1})
        shares = reward_config.get("shares", [1.0])
        reward_amounts = calculate_tiered_rewards(
...
def rank_participants_by_time(
    participants: List[Dict[str, Any]], time_field: str = "submitted_at"
) -> List[Dict[str, Any]]:
    """
    ...
    """
    return sorted(participants, key=lambda x: x.get(time_field, datetime.min))
...
def rank_participants_by_completion(
    ...
    """
    return sorted(participants, key=lambda x: x.get(completion_field, datetime.min))
...
def distribute_rewards_generic(
    ...
    if mode_id == "free_mode":
        ...
        if mode_config:
            return distribute_rewards_to_winners_five_dollar_mode(
                db, winners, mode_config, draw_date
            )

    return {"total_winners": len(winners), "total_rewarded": 0}
