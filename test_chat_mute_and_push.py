...
def test_chat_mute_endpoints():
    ...
    response = requests.post(
        f"{BASE_URL}/chat-mute/global", headers=headers_user1, json={"muted": True}
    )
    ...
    response = requests.post(
        f"{BASE_URL}/chat-mute/trivia-live", headers=headers_user1, json={"muted": True}
    )
    print_response("4. POST /chat-mute/trivia-live (mute)", response)

    # 5. Unmute global chat
    response = requests.post(
        f"{BASE_URL}/chat-mute/global", headers=headers_user1, json={"muted": False}
    )
...
def test_global_chat_with_push():
    ...
    if response.status_code == 200:
        ...
        print(
            "Note: Push notifications are sent in background to all users (except sender)"
        )
        print("      who are not muted and not active (30-second threshold)")

    # 2. Get global chat messages
    response = requests.get(
        f"{BASE_URL}/global-chat/messages?limit=10", headers=headers_user1
    )
...
def test_trivia_live_chat_with_push():
    ...
    response = requests.get(
        f"{BASE_URL}/trivia-live-chat/status", headers=headers_user1
    )
    ...
    if response.status_code == 200:
        ...
        if not is_active:
            print(
                "Note: Trivia live chat is not active. Messages can only be sent during active window."
            )
            return
    ...
    response = requests.get(
        f"{BASE_URL}/trivia-live-chat/messages?limit=10", headers=headers_user1
    )
...
def test_private_chat_with_mute():
    ...
    if response.status_code == 200:
        ...
        print(
            "Note: Push notification will be sent if user2 is not muted, not active, and has OneSignal player"
        )

    # 2. List private conversations
    response = requests.get(
        f"{BASE_URL}/private-chat/conversations", headers=headers_user1
    )
...
def main():
    ...
    try:
        ...
        print(
            "      Check your OneSignal dashboard to verify push notifications were sent."
        )
