    ...
    try:
        ...
        for i, user in enumerate(users):
            ...
            if i < 5 or i % 50 == 0:  # Only show first 5 and then every 50th
                print(f"User {user.account_id} ({user.username}): {user.profile_pic_url} -> {new_profile_pic}")
        ...
        # Commit all changes
        db.commit()
        print(f"Successfully updated {updated_count} profile pictures, skipped {skipped_count} users")
